...
不想敲命令时，用这个页面把①②③问串起来点着看（答辩/演示场景）：

```bash
pip install -r requirements.txt
streamlit run app.py          # Windows 也可双击 启动UI.bat
```

| 标签页 | 做什么 | 需要 API Key |
|---|---|---|
| ① Spec 解析 | 自然语言 → Spec JSON，可下载 | 在线模式需要；离线模式不需要 |
| ② 拓扑识别与参数约减 | 粘贴/上传网表 → 模块表 + 变量表 + variables.csv | 不需要 |
| ③ 尺寸优化与评分 | mock 仿真驱动 DE，看收敛曲线 / Pareto / 赛题评分自评 | 不需要 |
| ④ 环境自检 | 一键定位"页面起不来 / 解析失败"的原因 | 不需要 |

说明：
* API Key 只存在浏览器会话内存里，不写文件、不进日志、不进 Git；离线模式走本地规则解析。
* 第③问是 **mock 仿真演示**（验证工程链路用），数值没有物理意义，真实结果必须走九天服务器。
* 业务逻辑都在 `ui_logic.py`（不 import streamlit，可单独测试）；`app.py` 只负责渲染。
